...
- сервис для бесплатных SSL-сертификатов от `LetsEncrypt`:
  - используем стабильный образ `goacme/lego:v4.29.0`
  - добавляем логику слоем сверху
  - собираем `bitrix24/lego:4.29.0-v1-alpine`
...
docker pull alpine:3.22
docker pull goacme/lego:v4.29.0
```
...
- `bitrix24/lego`:
```bash
cd env-docker/sources/bxlego4290/
docker buildx build --platform linux/arm64,linux/amd64 --provenance=false -f Dockerfile -t bitrix24/lego:4.29.0-v1-alpine --no-cache .
```
