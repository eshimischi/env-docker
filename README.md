...
- сервис для бесплатных SSL-сертификатов от `LetsEncrypt`:
  - используем стабильный образ `goacme/lego:v4.27.0`
  - добавляем логику слоем сверху
  - собираем `bitrix24/lego:4.27.0-v1-alpine`
...
docker pull alpine:3.21
docker pull goacme/lego:v4.27.0
```
...
- `bitrix24/lego`:
```bash
cd env-docker/sources/bxlego4270/
docker buildx build --platform linux/arm64,linux/amd64 --provenance=false -f Dockerfile -t bitrix24/lego:4.27.0-v1-alpine --no-cache .
```
